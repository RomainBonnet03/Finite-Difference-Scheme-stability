...
from Lebarbenchon_codes.utils import boundary_to_boundary_quasi_toep, sort, epsilon, eta_func, parametrization
from Lebarbenchon_codes.boundaries import Dirichlet
...
class LaxWendroffInversed(Scheme):
    ''' Added by Romain Bonnet-Eymard in order to plot the figure 1.5 '''
    def __init__(self, lamb, boundary=Dirichlet(), **kwargs):
        self.sigma = kwargs.get("sigma", 0)
        self.lamb = lamb
        coeff3 = -(lamb - lamb**2) / 2
        coeff2 = 1 - lamb**2
        coeff1 = (lamb**2 + lamb) / 2
        self.int = np.array([coeff3, coeff2, coeff1])
        self.center = 1
        super().__init__(int=self.int, center=self.center, boundary=boundary, **kwargs)
